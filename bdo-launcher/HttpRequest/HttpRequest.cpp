...
#pragma comment(lib, "HttpRequest/3rd_party/libcurl/lib/libcurl_a.lib")
#pragma comment(lib, "HttpRequest/3rd_party/jsoncpp/lib/lib_json.lib")
#pragma comment(lib, "HttpRequest/3rd_party/zlib/lib/zlibstat.lib")
#pragma comment(lib, "HttpRequest/3rd_party/openssl/lib/libeay32.lib")
#pragma comment(lib, "HttpRequest/3rd_party/openssl/lib/ssleay32.lib")

#define ZLIB_WINAPI
#define CURL_STATICLIB

HTTPRequest::HTTPRequest() : m_hCurl(nullptr), m_header(nullptr)
{
	...
	if (pCode != CURLE_OK)
	{
		OutputDebugStringW(L"HTTPRequest: curl_global_init Failed...\r\n");
		return;
	}
	...
	if (!m_hCurl)
	{
		OutputDebugStringW(L"HTTPRequest:curl_easy_init Failed...\r\n");
		return;
	}

	//curl_easy_setopt(m_hCurl, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0);
	//curl_easy_setopt(m_hCurl, CURLOPT_ACCEPT_ENCODING, "gzip");
	//curl_easy_setopt(m_hCurl, CURLOPT_HTTP_CONTENT_DECODING, 1);
...
void HTTPRequest::sc_enableCookie()
{
	curl_easy_setopt(m_hCurl, CURLOPT_COOKIEJAR, "");
	//curl_easy_setopt(m_hCurl, CURLOPT_COOKIEFILE, "");
}

void HTTPRequest::sc_setCookieFile(const char* sFilename)
{
	if (!m_hCurl)
		return;
...
CURLcode HTTPRequest::sc_get(const char* sURL, const wchar_t* sFileName, bool bCaseRepHdr)
{
	if (!m_hCurl)
	...
	if (sFileName)
	{
		...
		if (err != 0)
		{
			OutputDebugStringW(L"HTTPRequest:Open File Failed...\r\n");
			return retCode;
		}
	}
	if (m_header)
...
CURLcode HTTPRequest::sc_get(const char* sURL, std::string& sRet, bool bCaseRepHdr)
{
	if (!m_hCurl)
		return CURLE_FAILED_INIT;
...
CURLcode HTTPRequest::sc_post(const char* sURL, const char* sData, const wchar_t* sFileName, bool bCaseRepHdr)
{
	if (!m_hCurl)
		return CURLE_FAILED_INIT;
...
CURLcode HTTPRequest::sc_post(const char* sURL, const char* sData, std::string& sRet, bool bCaseRepHdr)
{
	if (!m_hCurl)
		return CURLE_FAILED_INIT;
...
CURLcode HTTPRequest::sc_post(const char* sURL, const char* sFieldName, const char* sFileName, std::string& sRet, bool bCaseRepHdr)
{
	sRet.clear();
	...
	if (m_header)
		curl_easy_setopt(m_hCurl, CURLOPT_HTTPHEADER, m_header);
	CURLcode retCode = curl_easy_perform(m_hCurl);
	return retCode;
}

CURLcode HTTPRequest::sc_put(const char* sURL, const char* sData, std::string& sRet, bool bCaseRepHdr)
{
	if (!m_hCurl)
		return CURLE_FAILED_INIT;
...
void HTTPRequest::sc_cleanHeader()
{
	if (m_header)
...
void HTTPRequest::sc_appendHeader(const char* sHeader)
{
	m_header = curl_slist_append(m_header, sHeader);
}

std::string HTTPRequest::sc_getCookie(const char* sCookieName)
{
	std::string sRet("");
...
std::string HTTPRequest::sc_getCookie()
{
	std::string sRet("");
	struct curl_slist* cookies = NULL;
	if (!m_hCurl)
...
size_t HTTPRequest::_callBackWriteFile(void* buffer, size_t size, size_t nmemb, void* usrFp)
{
...
size_t HTTPRequest::_callBackNotCase(void* buffer, size_t size, size_t nmemb, void* usrFp)
{
	return size * nmemb;
}

size_t HTTPRequest::_callBackWriteString(void* buffer, size_t size, size_t nmemb, void* usrFp)
{
	size_t xSize = size * nmemb;
...
// 設置代理
void HTTPRequest::sc_setProxy(const char* sProxy, char* sPass, curl_proxytype ntype)
{
	if (!m_hCurl)
		return;
...
bool HTTPRequest::sc_readTextFile(const wchar_t* sFileName, string& sRet)
{
	sRet.clear();
...
std::string HTTPRequest::sc_getTimeStamp(int nOffest)
{
	struct timeb f;
	ftime(&f);
...
// 設置最大等待時間
void HTTPRequest::sc_setTimeOut(int nTimeOutMillisecond)
{
	if (!m_hCurl)
		return;
...
void HTTPRequest::sc_delCookieFile(void)
{
	if (m_szCookie)
		DeleteFileA(m_szCookie);
}

void HTTPRequest::sc_setGzip()
{
	curl_easy_setopt(m_hCurl, CURLOPT_ACCEPT_ENCODING, "gzip");
}

void HTTPRequest::sc_setHttpVersion(int version)
{
	curl_easy_setopt(m_hCurl, CURLOPT_HTTP_VERSION, version);
}

void HTTPRequest::sc_setCookie(const char* sName, const char* sValue)
{
	if (!m_hCurl)
		return;
...
void HTTPRequest::_checkExCookies()
{
	string sTmp("");
...
void HTTPRequest::sc_cleanExCookie()
{
	m_pCookieEx->clear();
}

bool HTTPRequest::sc_writeLog(const wchar_t* sFile, const char* sText)
{
	FILE* pFile = NULL;
...
int HTTPRequest::sc_getMidString(string src, string& dst, const char* sLeft, const char* sRight, int nStart)
{
	dst = "";
...
int HTTPRequest::sc_split(string str, string pattern, vector<string>& xRet)
{
	xRet.clear();
...
void HTTPRequest::sc_replace(string& str, const char* sRp, const char* sTo)
{
...
long HTTPRequest::sc_getResponseCode()
{
	long res = 0;
	curl_easy_getinfo(m_hCurl, CURLINFO_RESPONSE_CODE, &res);
	return res;
}

void HTTPRequest::sc_utf8ToAnsi(string& str)
{
	if (str.empty())
		return;
...
void HTTPRequest::sc_ansiToUtf8(string& str)
{
	if (str.empty())
		return;
...
void HTTPRequest::sc_ansiToUnicode(const string& str, wstring& sUnicode)
{
	if (str.empty())
		return;
...
void HTTPRequest::sc_unicodeToAnsi(const wstring& sUnicode, string& str)
{
	if (sUnicode.empty())
		return;
...
void HTTPRequest::sc_utf8ToUnicode(const string& sUtf8, wstring& sUnicode)
{
	string tmp(sUtf8);
	HTTPRequest::sc_utf8ToAnsi(tmp);
	HTTPRequest::sc_ansiToUnicode(tmp, sUnicode);
}

void HTTPRequest::sc_unicodeToUtf8(const wstring& sUnicode, string& sUtf8)
{
	HTTPRequest::sc_unicodeToAnsi(sUnicode, sUtf8);
	HTTPRequest::sc_ansiToUtf8(sUtf8);
}

bool HTTPRequest::sc_urlEncodeAnsi(string& strAnsiData, bool bUpperCase)
{
	string strEncode("");
...
bool HTTPRequest::sc_urlEncodeUtf8(string& strUtf8Data, bool bUpperCase)
{
	string strEncode("");
	char baseChar = bUpperCase ? 'A' : 'a';
	unsigned char c;
...
bool HTTPRequest::sc_urlDecodeUtf8(string& strUtf8Data)
{
	string strDecode("");
...
bool HTTPRequest::sc_urlDecodeAnsi(string& strAnsiData)
{
	string strDecode("");
...
string HTTPRequest::_base64DecodeRaw(const char* b, const char* e)
{
	static BYTE dec64[] =
	{
...
string HTTPRequest::_base64EncodeRaw(const char* b, const char* e)
{
...
string HTTPRequest::sc_base64Decode(const char* s, int len)
{
	if (s == NULL)
		return "";
...
string HTTPRequest::sc_base64Encode(const char* b, int len)
{
	if (b == NULL)
		return "";
	if (len == 0)
